...
function TextArrayInput(props) {
  ...
  function getRef(key) {
    if (!refsMap.current.has(key)) {
      refsMap.current.set(key, React.createRef());
    }
  ...
  function gatherValues() {
    ...
    [...Array(values.length).keys()].forEach((i) => {
      const ref = getRef(toKey(i));
      if (ref && ref.current && ref.current.lastChild.value.length > 0) {
        vals.push(ref.current.lastChild.value);
      }
    });

    const newRef = getRef(toKey("New"));
    if (newRef && newRef.current && newRef.current.lastChild.value.length > 0) {
      vals.push(newRef.current.lastChild.value);
    }
  ...
  function onKeyUp(e) {
    if (e.key === "Enter") {
      ...
      if (e.target.className === "text-array-input-new") {
        ...
        e.target.value = "";
        e.target.focus();
      } else if (getRef(toKey(e.target.tabIndex)).current) {
    ...
    if (e.key === "Backspace" && e.target.value.trim().length === 0) {
      ...
      if (
        e.target.tabIndex > 1 &&
        getRef(toKey(e.target.tabIndex - 2)).current
      ) {
    ...
    if (e.key === "ArrowDown") {
      const nextRef = getRef(toKey(e.target.tabIndex));
  ...
  return (
    <div className="text-array-input">
      {values.map((val, i) => {
        return (
          <label ref={getRef(toKey(i))} key={toKey(i + 1)}>
            <em>{i + 1}</em>
            <input
              type="text"
              defaultValue={val}
      ...
      <label
        ref={getRef(toKey("New"))}
        key={toKey(values.length + 1)}
        className="text-array-input-new"
      >
        <em>{values.length + 1}</em>
        <input
          className="text-array-input-new"
          type="text"
          onBlur={onChange}
